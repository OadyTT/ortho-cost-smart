// api/delete.js — ลบ row จาก sheet "บันทึก" ตาม rowIndex
import { google } from 'googleapis'

const SPREADSHEET_ID = '1IivnEuLFzonKTpC8WDMkCegUnTLde4JSwKYvwU0YJeU'
const CREDENTIALS = {
  client_email: 'ortho-sheet-writer@ortho-cost-smart.iam.gserviceaccount.com',
  private_key: '-----BEGIN PRIVATE KEY-----\\npHCJGBaJZltrIVib7FWm2DCO\n-----END PRIVATE KEY-----\n',
}

export default async function handler(req, res) {
  if (req.method !== 'DELETE') return res.status(405).end()

  // rowIndex = index ใน array (0-based, ไม่รวม header)
  // ใน sheet จริง = rowIndex + 2 (row 1 = header, row 2+ = data)
  const { rowIndex } = req.body

  if (rowIndex === undefined || rowIndex < 0) {
    return res.status(400).json({ error: 'Invalid rowIndex' })
  }

  try {
    const auth = new google.auth.GoogleAuth({
      credentials: CREDENTIALS,
      scopes: ['https://www.googleapis.com/auth/spreadsheets'],
    })
    const sheets = google.sheets({ version: 'v4', auth })

    // หา sheetId ของ sheet "บันทึก"
    const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID })
    const sheet = meta.data.sheets.find(s => s.properties.title === 'บันทึก')
    if (!sheet) return res.status(404).json({ error: 'Sheet บันทึก not found' })

    const sheetId = sheet.properties.sheetId
    // rowIndex+1 เพราะ row 0 = header ใน sheet (1-based index จาก row 2)
    const sheetRowIndex = rowIndex + 1  // 0-based sheet row (header=0, data starts at 1)

    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: {
        requests: [{
          deleteDimension: {
            range: {
              sheetId,
              dimension: 'ROWS',
              startIndex: sheetRowIndex,
              endIndex: sheetRowIndex + 1,
            }
          }
        }]
      }
    })

    res.json({ success: true })
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: err.message })
  }
}
